function [res] = histogram_matching()
  ...
  arr1 = calculate_histogram(imggray1);

  % Plot histogram for output image
  arr2 = calculate_histogram(imggray2);

  % Perform histogram matching
  imgnew1 = perform_histogram_matching(imggray1, arr1, arr2);
  ...
  subplot(2,3,6), bar(0:255, calculate_histogram(imgnew1), 1);
...
function histogram = calculate_histogram(img)
...
function img_matched = perform_histogram_matching(img, histogram_in, histogram_ref)
  cdf_in = cumsum(histogram_in) / sum(histogram_in);
  cdf_ref = cumsum(histogram_ref) / sum(histogram_ref);
  
  [~, map] = min(abs(cdf_in - cdf_ref'), [], 2);
  
  [nr, nc] = size(img);
  img_matched = zeros(nr, nc);
  for i = 1:nr
      for j = 1:nc
          img_matched(i, j) = map(img(i, j) + 1) - 1;
      end
  end
