function resimg = geometric_mean_filter()
  ...
  for i = 2:(nr-1)
    for j = 2:(nc-1)
       resimg(i,j) = (imdouble(i-1,j-1)*imdouble(i-1,j)*imdouble(i-1,j+1)*imdouble(i,j-1)*imdouble(i,j)*imdouble(i,j+1)*imdouble(i+1,j-1)*imdouble(i+1,j)*imdouble(i+1,j+1))^(1/9);
    end
  end
  ...
  title('After applying Geometric Mean Filter','FontSize',10);
